...
const RestaurantScreen: React.FC<RestaurantScreenProps> = ({          
  ...
  // User Geolocation State - FIXED: Remove time-based expiration, just load saved coordinates          
  const [userLat, setUserLat] = useState<number | null>(() => {      
    const saved = localStorage.getItem('howzeverything-user-location');      
    if (saved) {      
      try {      
        const { lat } = JSON.parse(saved);      
        console.log('📍 Restored saved location:', lat);      
        return lat;      
      } catch (e) {      
        console.log('📍 Failed to parse saved location, will test permission availability');      
      }      
    }      
    return null;      
  });          
  ...
  // FIXED: Start with true, will be set to false if location is available
  ...
  const [hasTestedPermission, setHasTestedPermission] = useState(false);
  ...
  const saveLocationToStorage = useCallback((lat: number, lon: number) => {      
    try {      
      const locationData = {      
        lat,      
        lon,      
        timestamp: Date.now() // Keep timestamp for reference, but not for expiration     
      };      
  ...
  // FIXED: Test current location permission availability on mount
  useEffect(() => {
    if (!navigator.geolocation || hasTestedPermission) {
      return;
    }

    setHasTestedPermission(true);
    console.log('📍 Testing current location permission availability...');

    // Test if we can get location silently (without showing banner)
    navigator.geolocation.getCurrentPosition(
      (position) => {
        // Success - location is available, don't show banner
        const newLat = position.coords.latitude;
        const newLon = position.coords.longitude;
        setUserLat(newLat);
        setUserLon(newLon);
        saveLocationToStorage(newLat, newLon);
        setShouldShowLocationBanner(false); // Hide banner since location is available
        setIsLocationPermissionBlocked(false);
        console.log('📍 Location is available, banner hidden:', newLat, newLon);
      },
      (error) => {
        console.log('📍 Location test result:', error.code, error.message);
        
        if (error.code === error.PERMISSION_DENIED) {
          // Permission explicitly denied - show banner and mark as blocked
          ...
          // Location unavailable but not necessarily blocked - show banner
          setShouldShowLocationBanner(true);
          setIsLocationPermissionBlocked(false);
          console.log('📍 Location unavailable, showing banner');
        } else {
          // Timeout or other error - could be temporary, show banner but not marked as blocked
          setShouldShowLocationBanner(true);
          setIsLocationPermissionBlocked(false);
          console.log('📍 Location timeout/error, showing banner');
        }
      },
      {
        enableHighAccuracy: false, // Use less accurate/faster for initial test
        timeout: 8000, // Reasonable timeout
        maximumAge: 300000 // Allow cached location up to 5 minutes for this test
      }
    );
  }, [hasTestedPermission, saveLocationToStorage]); // Only test once
    ...
    <div style={{ minHeight: '100vh', display: 'flex', flexDirection: 'column', backgroundColor: COLORS.background }}>          
      ...
      <main style={{          
        ...
        <div className="max-w-md mx-auto space-y-6"          
          ...
          {/* Location Permission Banner - Only show when location is not currently available */}      
