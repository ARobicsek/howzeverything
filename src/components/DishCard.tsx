    ...
    <div className="flex items-start justify-between mb-3">    
      ...
      <div className="flex items-center gap-1 sm:gap-2 flex-shrink-0" style={{ marginTop: '4px' }}>    
        ...
        <button    
          ...
          style={{
            backgroundColor: isMenuOpen ? '#3B82F6' : 'transparent',
            color: COLORS.text,
            minHeight: '32px',
            display: 'flex',
            alignItems: 'center',
...
const DishCard: React.FC<DishCardProps> = ({    
  ...
  // const [showAddCommentForm, setShowAddCommentForm] = useState(false); // REMOVED: Replaced by showCommentModal
  ...
  const [capturedPhoto, setCapturedPhoto] = useState<File | null>(null); // New state for captured photo
  const actionMenuRef = useRef<HTMLDivElement | null>(null);
  ...
  const handleAddCommentInternal = async (text: string) => {    
    ...
    // setShowAddCommentForm(false); // REMOVED: No longer necessary
  ...
  const handleTakePhoto = () => {
    ...
    input.type = 'file';
    input.accept = 'image/*';
    input.capture = 'environment'; // Use 'environment' for back camera
   
    input.onchange = (event: any) => {
      const file = event.target.files?.[0];
      if (file) {
        // Store the captured photo and show upload interface
  ...
  return (    
    <>
      {/* Grey overlay when action menu is open */}
      ...
      <div className="bg-white/5 backdrop-blur-sm p-5 rounded-lg hover:bg-white/10 transition-colors relative">    
        ...
        {/* Fanned out action buttons */}
        {showActionMenu && (
          <div ref={actionMenuRef} className="absolute z-50" style={{ top: '60px', right: '65px' }}>
            {/* Comments button */}
            <button
              onClick={() => {
              ...
              className="absolute p-3 rounded-full bg-white shadow-lg hover:shadow-xl transition-all"
              style={{
                top: '-10px',
                right: '50px',
        ...
        {showPhotoUpload && (    
          <div className="mt-3">    
            <PhotoUpload    
              ...
              onCancel={() => {
                setShowPhotoUpload(false);
                setCapturedPhoto(null); // Clear captured photo on cancel
              }}    
