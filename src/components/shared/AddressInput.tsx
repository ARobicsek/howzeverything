  ...
  return (
    <div ref={containerRef}>
      <div className="mb-2 relative">
        <label className="font-body block font-medium text-textSecondary mb-2">Full Address</label>
        <textarea
          ...
          className="w-full p-3 border-2 border-gray-200 rounded-md bg-white text-text min-h-20 resize-y"
          rows={3}
        />
        {(isFetchingSuggestions || suggestions.length > 0) && (
          <div className="absolute top-full left-0 right-0 bg-white border border-gray-200 rounded-md shadow-md z-dropdown max-h-52 overflow-y-auto">
            {isFetchingSuggestions && suggestions.length === 0 && (
              <div className="p-2 text-textSecondary font-body">Searching...</div>
            )}
            {suggestions.map((suggestion, index) => (
              <div
                key={index}
                onClick={() => handleSuggestionClick(suggestion)}
                className="px-3 py-2 cursor-pointer font-body hover:bg-gray-100"
              >
        ...
        <div className="min-h-5 pt-1">
          {isParsing && <p className="text-sm text-primary">Parsing...</p>}
          {parseMessage && <p className={`text-sm ${parseMessage.type === 'error' ? 'text-danger' : 'text-success'}`}>{parseMessage.text}</p>}
        </div>
      </div>
     
      {showParsedFields && (
        <div className="border-t border-border pt-3 mt-2">
          <p className="text-sm text-textSecondary mt-0 mb-3">
