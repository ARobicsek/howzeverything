...
interface PhotoUploadProps {  
  ...
  onCancel: () => void;  
  isUploading: boolean;  
  initialFile?: File; // For when photo is taken from camera
}

const PhotoUpload: React.FC<PhotoUploadProps> = ({ onUpload, onCancel, isUploading, initialFile }) => {  
  const [selectedFile, setSelectedFile] = useState<File | null>(initialFile || null);  
  const [preview, setPreview] = useState<string | null>(null);  
  const [caption, setCaption] = useState('');  
  const fileInputRef = useRef<HTMLInputElement>(null);

  // Handle initial file if provided (from camera)
  useEffect(() => {
    if (initialFile) {
          ...
          <div className="space-y-3">  
            ...
            <div className="space-y-2">  
              ...
              <input  
                ...
                className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:border-blue-500 focus:outline-none transition-all"  
                style={{  
                  ...FONTS.elegant,  
                  color: COLORS.textDark,  
                  fontSize: '0.95rem'  
            ...
            <div className="flex gap-2">  
              ...
              <button  
                ...
                disabled={isUploading}  
                className="px-4 py-2 rounded-lg border border-gray-300 hover:bg-gray-50 transition-colors focus:outline-none disabled:opacity-50 disabled:cursor-not-allowed"  
                style={{  
                  ...FONTS.elegant,  
                  color: COLORS.textDark,  
                  fontSize: '0.95rem',  
