...
export const useDishes = (restaurantId: string, sortBy: { criterion: 'name' | 'your_rating' | 'community_rating' | 'date'; direction: 'asc' | 'desc' }) => {  
    ...
    try {  
      ...
      setDishes(prev => sortDishesArray(prev.map(dish =>  
        dish.id === dishId  
          ? {  
              ...dish,  
              dish_comments: dish.dish_comments.map((comment: DishComment) =>  
    ...
    try {  
      ...
      setDishes(prev => sortDishesArray(prev.map(dish =>  
        dish.id === dishId  
          ? {  
              ...dish,  
              dish_comments: dish.dish_comments.filter((comment: DishComment) => comment.id !== commentId)  
            }  
          : dish  
      ), sortBy, currentUserId));  
    } catch (err: any) {  
  ...
  // ENHANCED: Add photo function with extended timeout and better error handling
  const addPhoto = async (dishId: string, file: File, caption?: string): Promise<void> => {
    ...
    try {
      ...
      console.log(`📤 Uploading compressed photo: ${fileSizeMB}MB`);

      // Enhanced upload with extended timeout and better error handling
      const uploadWithTimeout = new Promise<void>((resolve, reject) => {
        let uploadCompleted = false;
        
        // Set up a timeout that's longer for mobile networks (but shorter since files are compressed)
        const timeout = setTimeout(() => {
          if (!uploadCompleted) {
            reject(new Error('Upload timeout - please check your connection and try again'));
          }
        }, 90000); // 90 seconds timeout (reduced from 120 since compressed files are smaller)
        ...
        uploadPromise.then(({ error: uploadError }) => {
          ...
          if (uploadError) {
            ...
            if (uploadError.message.includes('The resource already exists')) {
              reject(new Error('A photo with this name already exists. Please try again.'));
            } else if (uploadError.message.includes('PayloadTooLarge')) {
              reject(new Error('File is too large. Please try a smaller image (under 5MB).'));
      ...
      // Get image dimensions (optional, with shorter timeout since files are smaller)
      ...
      if (file.type.startsWith('image/')) {
        ...
        const imgPromise = new Promise<void>((resolve) => {
          ...
          img.onerror = () => {
            // If image dimension detection fails, continue without dimensions
            resolve();
          };
        });
        img.src = URL.createObjectURL(file);
        
        // Add timeout for image dimension detection (shorter since compressed images load faster)
        await Promise.race([
          imgPromise,
          new Promise<void>((resolve) => setTimeout(resolve, 3000)) // 3 second timeout for dimension detection
        ]);
      }

      // Save metadata to database with timeout
      const dbSaveWithTimeout = new Promise<any>((resolve, reject) => {
        const timeout = setTimeout(() => {
          reject(new Error('Database save timeout. Photo uploaded but metadata save failed.'));
        }, 20000); // 20 second timeout for database operations (reduced since files are smaller)
        ...
        dbPromise.then(({ data: photoData, error: dbError }) => {
          clearTimeout(timeout);
          if (dbError) {
            reject(dbError);
          } else {
      ...
      const photoData = await dbSaveWithTimeout;
      ...
      if (photoData) {
        ...
        console.log(`✅ Photo upload successful: ${fileSizeMB}MB uploaded`);
      ...
      if (err.message.includes('timeout')) {
        errorMessage = err.message; // Use the specific timeout message
      } else if (err.message.includes('network') || err.message.includes('fetch')) {
        errorMessage = 'Network error. Please check your connection and try again.';
      } else if (err.message.includes('too large') || err.message.includes('PayloadTooLarge')) {
        errorMessage = 'File is too large. Image compression should have prevented this - please try again.';
      } else if (err.message) {
        errorMessage = `Failed to add photo: ${err.message}`;
      }
    ...
    try {  
      ...
      setDishes(prev => sortDishesArray(prev.map(dish =>  
        dish.id === dishId  
          ? {  
              ...dish,  
              dish_photos: dish.dish_photos.filter((photo: DishPhoto) => photo.id !== photoId)  
            }  
          : dish  
      ), sortBy, currentUserId));  
    } catch (err: any) {  
