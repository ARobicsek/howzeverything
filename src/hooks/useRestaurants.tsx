...
export const useRestaurants = (sortBy: 'name' | 'date') => {
  ...
  const searchRestaurants = async (query: string, userLat: number | null, userLon: number | null) => {
    ...
    try {
      ...
      // Renamed 'query' parameter to 'queryParam' to avoid potential conflicts/shadowing
      const processFeatures = (features: any[], queryParam: string, requireName: boolean = true) => {
        // To resolve 'queryParam' is declared but its value is never read (TS6133),
        // we'll make sure it's used within the function's logic.
        // In this case, we'll use it to filter initial results based on name/address
        // before the more precise similarity scoring happens later.
        const lowerCaseQueryParam = queryParam.toLowerCase().trim();
        ...
        return features
          .filter((feature: any) => {
            ...
            // Use queryParam to filter out irrelevant results early
            // This makes 'queryParam' used and addresses the TS6133 warning.
            // It also helps in reducing unnecessary processing for clearly non-matching results.
            if (lowerCaseQueryParam) {
                const nameMatch = properties.name?.toLowerCase().includes(lowerCaseQueryParam);
                const addressMatch = properties.formatted?.toLowerCase().includes(lowerCaseQueryParam);
      ...
      // Helper to detect if query contains both business and location
      const parseBusinessLocationQuery = (query: string): { business?: string; location?: string; isMultiPart: boolean } => {
        const lowerQuery = query.toLowerCase().trim();
        const words = lowerQuery.split(/\s+/);
       
        // Common business names that might be followed by a location
        const businessPatterns = [
          'starbucks', 'mcdonald', 'mcdonalds', 'burger king', 'subway', 'dunkin',
          'kfc', 'pizza hut', 'dominos', 'chipotle', 'wendys', 'taco bell',
          'chick-fil-a', 'panera', 'five guys', 'in-n-out', 'cafe', 'restaurant'
        ];
       
        // Check if first word(s) might be a business name
        for (let i = 1; i <= Math.min(3, words.length - 1); i++) {
          const potentialBusiness = words.slice(0, i).join(' ');
          const potentialLocation = words.slice(i).join(' ');
         
          // If we have a known business pattern, split there
          if (businessPatterns.some(pattern => potentialBusiness.includes(pattern))) {
            return {
              business: potentialBusiness,
        ...
        // Check if query looks like "business in location" pattern
        const inIndex = words.indexOf('in');
        const nearIndex = words.indexOf('near');
        const atIndex = words.indexOf('at');
       
        const splitIndex = inIndex !== -1 ? inIndex : (nearIndex !== -1 ? nearIndex : atIndex);
        if (splitIndex > 0 && splitIndex < words.length - 1) {
          return {
            business: words.slice(0, splitIndex).join(' '),
            location: words.slice(splitIndex + 1).join(' '),
            isMultiPart: true
          };
        }
      ...
      const isLocationSpecific = (query: string): boolean => {
        ...
        const locationKeywords = [
          /\bst(?:street)?\b/, /\bave(?:nue)?\b/, /\brd|road\b/, /\bblvd|boulevard\b/, /\bln|lane\b/,
      ...
      // STRATEGY 1: If we have a business + location query, use Places API with location center
      if (queryParts.isMultiPart && queryParts.business && queryParts.location) {
        console.log('🎯 Strategy 1 (Business + Location): First, geocoding the location:', queryParts.location);
       
        // First, geocode the location to get coordinates
        const locationGeocodeUrl = `https://api.geoapify.com/v1/geocode/search?text=${encodeURIComponent(queryParts.location!)}&limit=1&apiKey=${apiKey}`;
        const locationResponse = await fetch(locationGeocodeUrl);
       
        if (locationResponse.ok) {
          const locationData = await locationResponse.json();
          if (locationData.features && locationData.features.length > 0) {
            // Explicitly type locationCoords to resolve potential 'string | undefined' inference
            const locationCoords: { lat: number; lon: number; city?: string; name?: string; } = locationData.features[0].properties;
            ...
            // Use a radius search around the found location
            const radius = 10000; // 10km radius
            const placesUrl = `https://api.geoapify.com/v2/places?categories=${restaurantCategories.join(',')}&filter=circle:${locationCoords.lon},${locationCoords.lat},${radius}&limit=30&apiKey=${apiKey}`;
            console.log('🔍 Searching for businesses near location...');
           
            const placesResponse = await fetch(placesUrl);
            if (placesResponse.ok) {
              const placesData = await placesResponse.json();
              const places = processFeatures(placesData.features || [], query);
             
              // Filter places by business name
              const businessLower = queryParts.business.toLowerCase();
              const filteredPlaces = places.filter(place => {
                ...
                return nameLower.includes(businessLower) || businessLower.includes(nameLower) ||
                       calculateSimilarity(place.properties.name, queryParts.business!) > 70;
              });
             
              const scoredResults = filteredPlaces.map(r => ({
                ...r,
                similarity: calculateSimilarity(r.properties.name, queryParts.business!) + 20,
                distance: calculateDistance(biasLat, biasLon, r.properties.lat, r.properties.lon)
              }));
             
              allResults.push(...scoredResults);
              console.log(`✅ Strategy 1: Found ${scoredResults.length} ${queryParts.business} locations near ${queryParts.location}`);
            }
          }
        }
      }
      ...
      if (!queryParts.isMultiPart && initialQueryIsLocationSpecific) {
          ...
          const globalGeocodeResponse = await fetch(globalGeocodeUrl);
          ...
          if (globalGeocodeResponse.ok) {
              ...
              const globalRestaurantResults = processFeatures(globalGeocodeData.features || [], query, false); // Don't require name for geocoding
      ...
      const biasedGeocodeResponse = await fetch(biasedGeocodeUrl);
      ...
      if (biasedGeocodeResponse.ok) {
          ...
          const biasedRestaurantResults = processFeatures(biasedGeocodeData.features || [], query, false); // Don't require name
          ...
          if (queryParts.isMultiPart && queryParts.location) {
            scoredResults.forEach(result => {
              ...
              const locationLower = queryParts.location!.toLowerCase();
      ...
      if (allResults.length < 10 && !queryParts.isMultiPart) {
          ...
          if (placesResponse.ok) {
              const placesData = await placesResponse.json();
              const placesResults = processFeatures(placesData.features || [], query);
      ...
      // --- Final Processing: De-duplication, Scoring, and Sorting ---
      const uniqueResults = allResults.filter((item: GeoapifyPlace & { similarity: number, distance: number }, index: number, self: (GeoapifyPlace & { similarity: number, distance: number })[]) => {
          return index === self.findIndex((t: GeoapifyPlace & { similarity: number, distance: number }) => {
              ...
              return distance < 0.05 && ( // Within ~80 meters (0.05 miles)
                  calculateSimilarity(t.properties.name, item.properties.name) > 80 || // Very similar names
                  calculateSimilarity(t.properties.formatted, item.properties.formatted) > 80 // Very similar addresses
              );
          });
      });
      ...
      if (queryParts.isMultiPart || initialQueryIsLocationSpecific) {
          ...
          if (goodMatches.length > 0) {
              finalResultsForDisplay = goodMatches.slice(0, 20);
          ...
          if (veryGoodMatches.length >= 3) {
              ...
              finalResultsForDisplay = finalSortedResults.slice(0, 15);
              console.log('📍 Kept top 15 results for display.');
          }
      }
      ...
      if (finalResultsForDisplay.length === 0 && finalSortedResults.length > 0) {
          finalResultsForDisplay = finalSortedResults.slice(0, Math.min(5, finalSortedResults.length));
  ...
  const importRestaurant = async (selectedPlace: GeoapifyPlace) => {
    ...
    setRestaurantErrors(prev => {
      ...
      return newErrors; // Ensure the Map is returned
    });
      ...
      if (existingGlobalRestaurant) {
        ...
        if (alreadyFavorited) {
          setRestaurantErrors(prev => {
            ...
            return newErrors; // Explicitly return the new Map object
          });
          return false;
        }
